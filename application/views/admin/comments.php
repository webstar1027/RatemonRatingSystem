        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h3 class="page-header">Comments List</h3>
                </div>
            </div>
            <!-- /.row -->
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading"> <strong>Comments List</strong>
                        </div>
                                    ...
                                    <thead>
                                        ...
                                        <?php foreach ($comments_data as $row) { ?>
                                            <tr id="row_<?php echo $row['id']; ?>">
                                              ...
                                              <td><?php if(empty($row["name"])){ echo 'Deleted Name'; }else{ echo $row["name"]; } ?></td>
                                              <td><?php if(strlen($row["comment"]) > 100){ echo substr($row["comment"],0 , 100).'...'; ?> <a href='javascript:void(0)' onclick="view_comment('<?php echo base64_encode($row['comment']) ?>');" >Read all</a></span> <?php }else{ echo $row["comment"]; }  ?></td>
                                              <td><?php echo $row['ip_address']; ?> </td>
                                              <td><?php echo $row['date']; ?></td>
                                              <td>
                                                <div class="btn-group">
                                                    <a onclick="remove_comment('<?php echo $row['id']; ?>');" href="javascript:void(0)" class="btn btn-default btn-xs" title="Remove Comment"><i class="fa fa-trash-o fa-fw"></i></a>
                                                    <a onclick="edit_comment('<?php echo $row['id']; ?>');" href="javascript:void(0)" data-toggle="modal" data-target="#edit-comment-modal" class="btn btn-default btn-xs" title="Edit Comment"><i class="fa fa-pencil-square-o fa-fw"></i></a>
                                                </div>
                                              </td>
                                            </tr>
                                        <?php } ?>
